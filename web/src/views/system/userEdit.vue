    ...
    <div class="content-table">
      ...
      <el-form class="userAdd" :rules="moudelHandle.rules" ref="userForm" :model="moudelHandle.moudel" label-width="80px">
        <el-form-item label="名字" class="is-required" prop="name">
          <el-input v-model="moudelHandle.moudel.name" placeholder="请输入名字" ></el-input>
        </el-form-item>
        <el-form-item label="用户名" class="is-required" prop="userName">
          <el-input v-model="moudelHandle.moudel.userName" placeholder="请输入用户名" ></el-input>
        </el-form-item>
        <el-form-item label="密码" class="is-required" prop="userPwd">
          <el-input v-model="moudelHandle.moudel.userPwd" placeholder="请输入用户名" ></el-input>
        </el-form-item>
        ...
        <el-form-item label="可借书数" class="is-required" prop="balance" >
           <el-input v-model="moudelHandle.moudel.balance"  placeholder="请输入可借书数" :disabled="!checkLoginAccountName()"></el-input>
        </el-form-item>
      </el-form>
  ...
  export default {
      ...
      return {
        ...
        moudelHandle: {
          ...
          rules: {
            ...
            balance: [{ required: true, message: '请输入可借书数', trigger: 'blur' }]
          }
        }
      }
    },
    ...
    created () {
      if (this.$route.query) {
        ...
        this.moudelHandle.moudel = userInfo
      }
    },
    methods: {
      ...
      submitForm (formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
             if( this.moudelHandle.moudel.name.length!==0&&this.moudelHandle.moudel.userName.length!==0&&this.moudelHandle.moudel.userPwd.length!==0){
      ...
      saveUser (formName) {
        if (this.moudelHandle.moudel._id) {
          ...
          return this.$store.dispatch('addUser', this.moudelHandle.moudel).then(() => {
            this.$message({
              message: '新增用户成功',
              type: 'success'
            })
